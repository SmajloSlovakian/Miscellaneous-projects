...
def lopticka(o):#sledovanie stlačenia a vykreslenie lopty o
    ...
    for i in range(0,prekazky):
        if prekazkaXb[i]-1>=x[o]+r[o]>=prekazkaXa[i]+1 and prekazkaYb[i]-1>=y[o]>=prekazkaYa[i]+1 or prekazkaXb[i]-1>=x[o]-r[o]>=prekazkaXa[i]+1 and prekazkaYb[i]-1>=y[o]>=prekazkaYa[i]+1:
            if fx[o]>=fNaDeform:
                predX[o]=(2/60)*fps
        elif prekazkaXb[i]-1>=x[o]>=prekazkaXa[i]+1 and prekazkaYb[i]-1>=y[o]+r[o]>=prekazkaYa[i]+1 or prekazkaXb[i]-1>=x[o]>=prekazkaXa[i]+1 and prekazkaYb[i]-1>=y[o]-r[o]>=prekazkaYa[i]+1:
            if fy[o]>fNaDeform:
                predY[o]=(2/60)*fps
                #lopty
    for i in range(0,lopty):
        if x[i]+r[i]-1>=x[o]+r[o]>=x[i]-r[i]+1 and y[i]+r[i]-1>=y[o]>=y[i]-r[i]+1 or x[i]+r[i]-1>=x[o]-r[o]>=x[i]-r[i]+1 and y[i]+r[i]-1>=y[o]>=y[i]-r[i]+1:
            if fx[o]>fNaDeform:
                predX[o]=(2/60)*fps
        if x[i]+r[i]-1>=x[o]>=x[i]-r[i]+1 and y[i]+r[i]-1>=y[o]+r[o]>=y[i]-r[i]+1 or x[i]+r[i]-1>=x[o]>=x[i]-r[i]+1 and y[i]+r[i]-1>=y[o]-r[o]>=y[i]-r[i]+1:
            if fy[o]>fNaDeform:
                predY[o]=(2/60)*fps
                #steny
    if x[o]+r[o]>=prX+1 or x[o]-r[o]<=minX-1:
        ...
        if fy[o]>fNaDeform:
            predY[o]=(2/60)*fps
            #kurzorhit
    if cursorX+cursorR>=x[o]+r[o]>=cursorX-cursorR and cursorY+cursorR>=y[o]>=cursorY-cursorR or cursorX+cursorR>=x[o]-r[o]>=cursorX-cursorR and cursorY+cursorR>=y[o]>=cursorY-cursorR:
        if fx[o]>fNaDeform:
            predX[o]=(2/60)*fps
    elif cursorX+cursorR>=x[o]>=cursorX-cursorR and cursorY+cursorR>=y[o]+r[o]>=cursorY-cursorR or cursorX+cursorR>=x[o]>=cursorX-cursorR and cursorY+cursorR>=y[o]-r[o]>=cursorY-cursorR:
        if fy[o]>fNaDeform:
            predY[o]=(2/60)*fps
            #vykreslenie loptyV
    if predX[o]>=1:
...
def kolizie():#vypočítanie sily a miesta pri kolízii
    ...
    if cursorhit==1: #kolízie s kurzorhitom
        if cursorX+cursorR>=x[o]>=cursorX-cursorR:
            if cursorY+cursorR>=y[o]+r[o]>=cursorY-cursorR:
            ...
            elif cursorY+cursorR>=y[o]-r[o]>=cursorY-cursorR:
                fy[o]=fy[o]*(-1)/(absorbcia[o])
                y[o]=cursorY+cursorR+r[o]
                indexfx=fx[o]-cursorFx
                indexfy=fy[o]-cursorFy
                fy[o]=(fy[o]+abs(indexfy))
                fx[o]=fx[o]-indexfx/200*treniecur
        elif cursorY+cursorR>=y[o]>=cursorY-cursorR:
            if cursorX+cursorR>=x[o]+r[o]>=cursorX-cursorR:
            ...
            elif cursorX+cursorR>=x[o]-r[o]>=cursorX-cursorR:
                fx[o]=fx[o]*(-1)/(absorbcia[o])
                x[o]=cursorX+cursorR+r[o]
                indexfx=fx[o]-cursorFx
                indexfy=fy[o]-cursorFy
    ...
    for i in range(0,prekazky): #kolízie s prekážkami
        if prekazkaXb[i]>=x[o]>=prekazkaXa[i]:
            if prekazkaYb[i]>=y[o]+r[o]>=prekazkaYa[i]:
                fy[o]=fy[o]*(-1)/(absorbcia[o])
                y[o]=prekazkaYa[i]-r[o]
            elif prekazkaYb[i]>=y[o]-r[o]>=prekazkaYa[i]:
                fy[o]=fy[o]*(-1)/(absorbcia[o])
                y[o]=prekazkaYb[i]+r[o]
        elif prekazkaYb[i]>=y[o]>=prekazkaYa[i]:
            if prekazkaXb[i]>=x[o]+r[o]>=prekazkaXa[i]:
                fx[o]=fx[o]*(-1)/(absorbcia[o])
                x[o]=prekazkaXa[i]-r[o]
            elif prekazkaXb[i]>=x[o]-r[o]>=prekazkaXa[i]:
                fx[o]=fx[o]*(-1)/(absorbcia[o])
...
while lowpower==0:
    ...
    pr.update()
    sleep(nonneg(ms/1000-(time()-tick)))
    tick=time()
    pr.delete('all')
